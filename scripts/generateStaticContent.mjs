 ...
 * This runs after the build process to inject pre-rendered content into index.html
 */
...
// Check if we're in production build (index.html exists in dist)
...
try {
  ...
  console.log('⚠️ Statický obsah sa negeneruje (možno development build)');
  console.log('   Toto sa spustí automaticky pri production build');
}